...
def main():
    ...
    path_model = 'best_model_conv_v1_S.pt'
    ...
    with torch.no_grad():
        for batch in bar:
            ...
            mse = mean_squared_error(labels_desnorm[0], pred_desnorm[0])
            rmse = np.sqrt(mse)
            mae = mean_absolute_error(labels_desnorm[0], pred_desnorm[0])
            r2 = r2_score(labels_desnorm[0], pred_desnorm[0])

            mean_error_SBP, std_error_SBP = aami_metrics(labels_desnorm[0], pred_desnorm[0])
            ...
            all_preds.append(pred_desnorm[0])
            all_labels.append(labels_desnorm[0])
            ...
            for j in range(pred_desnorm.shape[0]):
                ...
                valores_errores.append(error_sbp)
