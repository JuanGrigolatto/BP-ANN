...
def main(num_tasks=10000 ,tasks_per_batch=4, adapt_lr=0.01, meta_lr=0.005, k_adapt_steps=10, seed=42, num_epochs=100, N_patient_group = 4, p_support = 5, q_query= 10, base_dataset=None, selected_patients=None,
    ...
    model=Modelo_ConvolucionalV1(in_channels=2,out_channels=1, long_signal=500).to(device)
    ...
    label_idx = 0 if pressure_estimation.lower() == 'sbp' else 1
    print(f"Entrenando solo para estimar: {pressure_estimation.upper()} (índice {label_idx})")
    ...
    for epoch in range(num_epochs):  
        ...
        for iter, batch in enumerate(tqdm(dataloader)):
            ...
            # Seleccionar solo la presión objetivo
            y_support = y_support[..., label_idx].unsqueeze(-1)
            y_query   = y_query[..., label_idx].unsqueeze(-1)
            ...
            if meta_train_loss < best_valid_loss:
                ...
                }, f'models/best_meta_models/best_meta_model_{pressure_estimation}_patientwise.pt')
